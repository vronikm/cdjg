<?php
	...
	class reporteController extends mainModel{	
		...
		public function listarPagosConsolidado($fecha_inicio, $fecha_fin){
			...
			$datos = $this->ejecutarConsulta($consulta_datos);
			$datos = $datos->fetchAll();
			foreach($datos as $rowssuma){
				$VALOR_PAGADO += $rowssuma['VALOR_PAGADO'];
				$VALOR_PENDIENTE += $rowssuma['VALOR_PENDIENTE'];				
			}
			$columnas = 12; // total real de columnas
			$tabla.='
				<tr>
					<td colspan="'.($columnas-4).'" class="text-end fw-bold " style="text-align: right">TOTAL</td>
					<td class="bg-warning text-end fw-bold" style="text-align: right">'.number_format($VALOR_PAGADO,2,'.',',').'</td>
					<td class="bg-info text-end fw-bold" style="text-align: right">'.number_format($VALOR_PENDIENTE,2,'.',',').'</td>
					<td></td>
					<td></td>
				</tr>';
				$VALOR_PAGADO = 0;
